...
**第三、四轮验收变更**（附录 E）：注释头挪到末行、`🌍 国外代理` 改名 `🌍 国际代理`、可创建空分组卡片、小卡片可跨集合改父与重排、可在灵活桶里独立成组（`standalone`）、拖拽落点判定放宽、大卡片默认收起。
...
interface RuleCard {
  ...
  order: number                    // 桶内排序
  standalone?: boolean             // 小卡片：与父卡片同桶时也自己算一个输出单元
  note?: string
}
...
一个桶里"算一个输出单元"的卡片称为**顶层卡片**，三种情况：

- 大卡片，恒是
- 小卡片，父卡片**不在**同一个桶里（它被单独拖出来了）
- 小卡片，父卡片同桶，但它标了 `standalone: true`

只有"父卡片同桶且没标 `standalone`"的小卡片不是顶层卡片——它的来源已由父卡片的 `effectiveSources()` 收进去，单独再算一次会重复输出。

**`standalone` 是为灵活桶准备的**（附录 E.5）。那里每张顶层卡片各自成一个策略组，因此用户需要能把某一张小卡片从集合里拎出来单独成组——例如 `🤖 AI 服务` 整体一组、但 `💠 Gemini` 单独一组走另一个出口，同时集合里其余小卡片照旧合并。缺了它，"小卡片与父卡片同桶"就被硬编码成"必须合并"，策略组的精细程度封顶。

承接桶里这个标记不改变输出（整桶汇进同一个组），只影响卡片显示在哪一层，因此 UI 只在灵活桶给这个开关。

```ts
// catalog.js —— 判定与取来源都收在这里，四处调用同一份口径
isTopLevelIn(cards, card, bucket): boolean
representedChildren(cards, card): RuleCard[]   // 同桶且未 standalone 的小卡片
effectiveSources(cards, card): CardSource[]
// 小卡片 → 自身 sources
// 大卡片 → 自身 sources（恒空）+ 其同桶小卡片的 sources，按 order 排列
```

`serialize.js` 的 `topLevelCardsIn()`、`validate.js` 的 `orderedActiveCards()` 与 `checkFlexibleNames()`、`RuleGeneratorModal` 的 `orderedActiveCards`、`BucketPanel.vue` 的 `topLevelIn()` 原先各写一遍这个判定——改一处漏三处。加 `standalone` 时一并收拢到 `isTopLevelIn()`，五处统一调它。
...
**拖出 `🎯 全球直连` 要给警告**：`GEOIP,CN` 覆盖面极大，一旦落进灵活桶或 `🌍 国际代理`，它会排在 `🎯 全球直连` 的全部域名规则之前，把 `ChinaDomain` 之类清单整体遮蔽。这是 `warn` 级提示而非拦截——用户可能确有此意图（如刻意让国内 IP 走代理）。判定口径是"该卡片的全部来源都是 `GEOIP` / `IP-CIDR` / `IP-CIDR6` 类型"，域名与 IP 混合的卡片不触发。
...
#### 4.5.2 注释头只活在 MiSub 里

**它不会进入客户端拿到的配置。** 两道保证：

1. `ini-template-parser.js:10` 跳过 `;` 与 `#` 开头的行——注释根本没进模板模型
2. 六个渲染器都是**从模型重新拼产物**（`render-clash.js` 输出 YAML、`render-singbox.js` 输出 JSON……），不存在把 INI 原文透传的路径

`rule-generator-render-matrix.test.js` 固定了这条：把带注释头的 INI 过六个渲染器，断言产物里既没有 `misub-visual-state`、也没有那串 base64 的任何片段。

因此注释头对订阅侧完全无害，只是模板正文里多一行；序列化把它放在**最后一行**（4.5.1），高级模式的文本框第一行仍是 `[custom]`。
...
固定顺序"`🌍 国际代理` → `🎯 全球直连`"本身会制造问题：`Google.list` 含 `DOMAIN-KEYWORD,google`，会吞掉 `🎯 全球直连` 里 `GoogleCN.list` 的 `google.cn`。这正是 ACL4SSR 必须把 GoogleCN 置顶的原因（附录 A）。
...
因此四类承接组（灵活桶各组 / `🌍 国际代理` / `🎯 全球直连` / `🐟 漏网之鱼`）**成员完全一致**，都是「桶标准成员」：`🚀 节点选择` 打头，其后是用户实际勾选了的可选基础组，末位 `DIRECT`。
...
右栏六段里有四段各自对应策略组：灵活桶（每张顶层卡片一组）、`🛑 广告拦截`、`🌍 国际代理`、`🎯 全球直连`；`🔧 前置修正` 不产生组（指向字面量 `DIRECT`），`🐟 漏网之鱼` 恒定产生组但不接受拖放。
...
ruleset=🌍 国际代理,$ACL/Ruleset/Telegram.list
ruleset=🌍 国际代理,$ACL/Ruleset/Google.list
...
custom_proxy_group=🌍 国际代理`select`[]🚀 节点选择`[]☑️ 手动切换`[]♻️ 自动选择`[]DIRECT
...
**规则段**对照 5.1 的六段：`LocalAreaNetwork` 是局域网直连开关，`GoogleCN` / `SteamCN` / `UnBan` 是 `✅ 直连例外` 大卡片的三张小卡片——四行全部指向字面量 `DIRECT`；`🎮 我的游戏`（用户）排在 `🤖 AI 服务`、`🎬 流媒体`（内置）之前；`BanAD` 系列在 `🛑 广告拦截`；`Telegram` 与 `Google` 在 `🌍 国际代理`；`🏠 国内直连` 的五张小卡片在 `🎯 全球直连`，`ChinaCompanyIp`（`order: 998`）与 `GEOIP,CN`（`order: 999`）钉在段尾。

**组段**对照 5.2.4 的十段：基础组两个（未勾选故障转移）、地区组五个、灵活桶三组、广告拦截、国际代理、全球直连、漏网之鱼。
...
三个灵活桶组与 `🌍 国际代理`、`🐟 漏网之鱼` 的成员**逐字相同**（5.2.1：出口不在卡片上写死）。`🎯 全球直连` 只是把 `DIRECT` 提到首位。`🛑 广告拦截` 是唯一另一套成员。
...
- **卡片名唯一性**：进入灵活桶的**顶层卡片**名不得与地区组、基础组（`🚀 节点选择` / `☑️ 手动切换` / `♻️ 自动选择` / `🔯 故障转移`）、承接组（`🛑 广告拦截` / `🌍 国际代理` / `🎯 全球直连` / `🐟 漏网之鱼`）、其它灵活桶顶层卡片重名，也不得与 `DIRECT` / `REJECT` 等保留字冲突。`dedupeGroupsByName`（`template-processor.js:102-137`）会**静默合并**同名组并合并其成员与 options，语义被改变而用户无感。判定必须走 `effectiveSources()` 而非 `card.sources`——大卡片自身 sources 恒空，否则会漏检
...
上面的示意图画的是**已配置好且各卡片都展开**的状态。初始状态是：右栏五个可拖放段全空（`🐟 漏网之鱼` 恒定但无可配置项），左栏 10 节全部收起。五个可拖放段因此**默认全展开**——段收起时它的 draggable 整个不渲染、拖不进去，而拖进来是唯一的填充方式。

**大卡片的小卡片列表两栏都默认收起**，点卡片左侧那个小三角展开——它长在卡片里、占原先 `⋮⋮` 把手的位置（把手在"整张卡片可拖"之后就没有功能了，已移除）。收起时列表仍然接放：`model-value` 给空数组、靠 `emptyInsertThreshold` 命中，放进去之后**自动展开**——否则拖进去的卡片会像凭空消失。角标始终显示里面有几张。
...
**灵活桶里的小卡片多一个角标钮**：与父卡片同桶时显示「独立成组」，点一下它就自己成一个策略组、并移到段的顶层列表；已独立的显示「并入集合」，点一下并回去（4.2.1 / 附录 E.5）。拖拽也能达到同一效果——拖进段的顶层列表即独立，拖回集合的小卡片列表即并入——按钮的意义在于它在窄屏与折叠状态下同样可用，且写明了这个操作叫什么。
...
阶段 A + B 与 C0 已全部满足，`npm run test:run` 116 文件 / 694 用例全绿，`npm run build` 通过。
...
8. ✅ 灵活桶各组 / `🌍 国际代理` / `🐟 漏网之鱼` 的成员**逐字相同**，都是「桶标准成员」= `🚀 节点选择` + 已勾选的手动切换/自动选择/故障转移 + `DIRECT`；地区组不进桶成员；**卡片上没有出口目标**
...
## 附录 E：第三、四轮验收变更（2026-09-02）

### E.1 交互与文案

| 变更 | 说明 |
|---|---|
| 注释头挪到**最后一行** | 首行是一串 base64 时，高级模式的文本框第一眼全是乱码。位置对渲染器无所谓（跳过 `;` 行），`readStateHeader` 改为扫全文，历史模板把头放首行的照旧能读（4.5.1） |
| 「💡 为什么要按国家分流？」整块移除 | 验收要求。两条 i18n 键一并删除 |
| 「🧱 自定义规则集」可提交**空分组卡片** | 一行规则都不填、只起个名字即可，得到一张空的大卡片，用来自己攒集合。名字必填——空卡片总得有个称呼（7.2） |
| `🌍 国外代理` 改名 `🌍 国际代理` | `parse.js` 留了 `LEGACY_POLICY_NAMES` 别名表：旧组名仍然认，否则早先生成的模板一反推就会把那一段变成灵活桶里的一张用户卡片、凭空多一个组 |
| 目录里 11 张卡片补拉丁品牌名 | `🎵 声破天` 这类音译名认不出来是 Spotify。改为 `🎵 Spotify 声破天` 等；卡片名同时是灵活桶下的策略组名，因此只补名不改结构 |
| 大卡片的小卡片列表**默认收起**，展开钮长在卡片里 | 见 7.2。收起时列表仍接放，放进去自动展开 |
| `⋮⋮` 把手移除 | 整张卡片可拖之后它没有功能了，只是挤掉一格宽度。展开钮接管它原来的位置 |
| 窄屏的「移到…」下拉挪到卡片**右下角** | 原先在左下角。它现在排在来源清单与备注之后，是卡片里最后一个可操作元素，手机上拇指最容易够到 |

### E.2 拖拽手感（验收原话："判定范围比较小，拉到右边得比视觉上更靠右"）

| 变更 | 为什么 |
|---|---|
| 去掉 `handle`，整张卡片可拖 | 原先只有 `⋮⋮` 那个小把手能拖，指针因此恒在卡片最左侧，而 SortableJS 的落点判定看的是**指针**位置——"卡片看着已经进右栏、其实指针还在左栏"就是这么来的。卡片里的按钮与下拉由 `filter: 'button, select, input, a, .no-drag'` 排除，`preventOnFilter: false` 让它们的点击照常 |
| `emptyInsertThreshold` 5px → 44px | 默认值下空列表几乎要贴着边才认 |
| 左栏的**尾部空白**接放，不加任何可见控件 | 原先左栏只有各节内部那几条窄列表能接放，空白处放不下去。曾试过在顶部加一条 sticky 的「拖到这里放回待选栏」，验收判定它多余（还会在窄屏出现，而窄屏根本不启用拖拽），改为一块 `flex-1` 的隐形接放区，窄屏下整块不渲染 |
| 列表缩进从 `margin` 改 `padding` | 可接放区域因此横跨整栏，而不是只有右边那一截 |
| 打开 `scroll` / `bubbleScroll`（sensitivity 90 / speed 14） | 拖拽时容器自动滚动 |

### E.3 小卡片重排与改父（验收原话："在大卡片中拖拽无法改变小卡片顺序"）

- 左栏节内小卡片改成**按 `order` 排**。原先拖完 `order` 确实写进去了，但渲染仍按数组顺序，所以看着像没生效
- 右栏大卡片下面挂的是**独立的可拖放列表**，不再是 `RuleCardItem` 内嵌的只读清单
- **拖进任意大卡片的小卡片列表 = 改父**（`handleChildDrop`），并跟到那张大卡片所在的桶。因此小卡片可以在集合之间搬家，不必再走"拉到右侧 → 滚动 → 拉回左侧"。大卡片被拖进小卡片列表时不改父，只跟着改桶——嵌套只有两层（4.2）

### E.4 一次被单元测试漏掉的 dev-only 崩溃

`BucketPanel.vue` 的 `#item` 插槽里多写了一行 HTML 注释，vuedraggable 于是抛 `Item slot must have only one child`——注释也算一个节点。**只在 dev 现形**：Vue 生产构建把注释编译掉，而其余组件用例把 `vuedraggable` 换成了静态壳，两边都测不到。

已补一条**用真 `vuedraggable` 挂载整个 modal** 的用例，并把注释重新注进去确认它会红、再撤掉。`BucketPanel.vue` 里也写下了这条坑。

### E.5 小卡片可在灵活桶里独立成组（`standalone`）

**验收原话**："子卡片和父卡片位于同一个桶中时，会被强制归回到父卡片中……灵活桶内必须要能够让子卡片独立存在于父卡片之外，因为这涉及到策略组的详细程度。"

原模型把"小卡片与父卡片同桶"直接等同于"必须合并进父卡片那一组"，于是策略组的精细度封顶：想让 `💠 Gemini` 单独成组、同时 `🤖 AI 服务` 里其余小卡片仍合并，做不到——把 Gemini 拖到段的顶层列表也会被判回父卡片。

加 `RuleCard.standalone`（4.2 / 4.2.1）：

- **顶层判定**收拢进 `catalog.js` 的 `isTopLevelIn()`，五处调用点统一；`representedChildren()` 同步排除 standalone 小卡片，避免同一条规则输出两次
- **三条入口**：卡片上的「独立成组 / 并入集合」角标钮（只在灵活桶、且父卡片同桶时出现）、把小卡片拖进段的顶层列表、以及反推
- **取消**：拖回集合的小卡片列表、退回待选栏，都清掉该标记（待选栏按父子分节展示，不体现独立成组）
- 承接桶里标了也不改变输出（整桶汇进同一组），因此 UI 不在那里给开关
- 注释头新增 `standalone` 字段（`CARD_FIELDS`）；只在为 true 时写入，取消时 `delete` 掉，避免 78 张卡片各多记一条 `false`

顺带修掉一个反推漏洞：正文里同时有 `ruleset=🤖 AI 服务,…` 与 `ruleset=💠 Gemini,…` 时，Gemini 组名等于小卡片名，反推后它与同桶的父卡片挤在一起、被父卡片吞掉，那个策略组就没了。现在这种情况直接标 `standalone`，两个组都留住。

---

**文档版本**：2.0（第四轮验收同步稿）
**最后更新**：2026-09-02
**状态**：阶段 A + B 与 C0 已实现，`npm run test:run` 116 文件 / 694 用例全绿，`npm run build` 通过；阶段 C 余项待排期，C6（一键设定规则分组的界面入口）为下一个候选。拖拽手感（C5）已按 E.2 调过一轮，仍需人工复核
