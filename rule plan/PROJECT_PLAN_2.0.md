...
**第三、四轮验收变更**（附录 E）：注释头挪到末行、`🌍 国外代理` 改名 `🌍 国际代理`、可创建空分组卡片、小卡片可跨集合改父与重排、拖拽落点判定放宽、大卡片默认收起。
...
阶段 A + B 与 C0 已全部满足，`npm run test:run` 116 文件 / 684 用例全绿，`npm run build` 通过。
...
**文档版本**：2.0（第四轮验收同步稿）
**最后更新**：2026-09-02
**状态**：阶段 A + B 与 C0 已实现，`npm run test:run` 116 文件 / 684 用例全绿，`npm run build` 通过；阶段 C 余项待排期，C6（一键设定规则分组的界面入口）为下一个候选。拖拽手感（C5）已按 E.2 调过一轮，仍需人工复核
