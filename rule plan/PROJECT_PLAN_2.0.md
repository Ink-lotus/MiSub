...
**文档状态**：阶段 A + B 已实现并通过本地验收。本文已回写实现期间的全部设计变更，与代码同步。
...
9 张大卡片，30 张小卡片。默认落点写在大卡片上，小卡片默认跟随父卡片；标了 `off` 的小卡片留在待选栏，需用户手动拖入。

| 大卡片 | 默认桶 | 小卡片 |
|---|---|---|
| `✅ 直连例外` | `prepend` | 谷歌中国 / Steam 中国 / 误杀捞回 / 谷歌推送(off) |
| `🛑 广告过滤` | `adblock` | 广告基础 / EasyList 增强(off) |
| `🤖 AI 服务` | `flexible` | OpenAI / Claude / 其它 AI |
| `🎬 流媒体` | `flexible` | 油管 / 奈飞 / HBO(off) / 巴哈姆特(off) / 声破天(off) / 抖音国际(off) / 国际媒体合集(off) |
| `📲 社交通讯` | `proxy` | 电报消息 / 推特(off) |
| `💻 科技服务` | `proxy` | 谷歌服务 / 微软服务(off) / 苹果服务(off) |
| `🎮 游戏平台` | `off` | Steam 平台 / Epic 平台 |
| `🏠 国内直连` | `direct` | 中国域名 / 国内媒体 / 下载工具 / 国内厂商 IP / 国内 IP |
| `🌏 广覆盖代理清单` | `off` | GFW 清单 / 精简代理清单(off) |

大卡片名刻意与 `GROUP_NAMES` 全部不同——大卡片进灵活桶后其 `name` 即策略组名，撞名会被 `dedupeGroupsByName`（`template-processor.js:102-137`）静默合并（6.2）。

`✅ 直连例外` 而非"CN 例外"：其四张小卡片的共性是"必须先直连、别被后面的规则吞掉"，而 `UnBan.list` 本身不是 CN 清单（4.1）。

**目录内全部 URL 与其在 `render-clash.js:64-99` 下的重写目标已逐条 HEAD 探测**（2026-08-30，全部 200）。两处与本文初稿不符、已按实际情况修正：

- `SteamCN.list` 与 `GoogleFCM.list` 在 `Clash/Ruleset/` 下，root 路径为 404
- Disney / GitHub / PayPal / PrimeVideo / GameDownloadCN / Copilot 在 ACL4SSR 中**不存在**，未收入目录

**已知粗糙点（待后续会话调整）**：目录的分组粒度偏粗——`🎬 流媒体` 7 张小卡片里 5 张默认关闭、`💻 科技服务` 把谷歌与微软/苹果并列但默认落点不同。这属于策展质量而非模型问题，不影响 4.2 的结构。
...
因此它和其它卡片一样可以拖回左侧待选栏（等于关掉），也可以单独拖到别的桶。`order: 999` 让它默认钉在所属桶的末尾，满足"IP 规则排在域名规则之后"的位置约定。同桶的 `🏢 国内厂商 IP` 用 `order: 998` 紧挨其前。
...
`[]GEOIP,CN` 不是独立一段。它是 `🏠 国内直连` 下的一张小卡片（4.3），默认落在 `🎯 全球直连` 且 `order: 999`，因此自然排在该组所有域名规则之后、`[]FINAL` 之前，位置约定不变。
...
现在由「🔧 前置修正」段接管：它是右栏第一段、接受拖放，段内规则**全部指向字面量 `DIRECT`**，语义就是"最高优先级且强制直连"。GoogleCN / SteamCN / 误杀捞回作为 `✅ 直连例外` 大卡片的小卡片默认落在这里。
...
默认状态另加一张用户自建的 `🎮 我的游戏`（1 条远程 + 1 条内联，落在灵活桶）。为便于阅读，下面把 `https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash` 缩写为 `$ACL`；实际输出是完整 URL。
...
- ✅ **A1** `src/utils/rule-generator/catalog.js`（375 行）
  9 张大卡片 + 30 张小卡片 + 6 个地区预置。大卡片 `sources` 恒空，规则绑在小卡片上（4.2.2）。地区 pattern 存**内层形式**、序列化时统一包一层外括号，满足 3.1 约束。含 `geoip-cn` 小卡片（`order: 999`，挂在 `🏠 国内直连` 下）。另导出 `effectiveSources()` / `childrenOf()` 两个供三处共用的口径函数（4.2.1）。
- ✅ **A2** `src/utils/rule-generator/serialize.js`（约 300 行）
  `GeneratorState → INI`。含 `; misub-visual-state-v1:` 注释头。规则按 5.1 六段顺序、组按 5.2.4 十段顺序输出。`topLevelCardsIn()` 决定哪些卡片算一个输出单元；`flexible` 建新组，`prepend` 指向字面量 `DIRECT`，其余并入各自承接组。四类承接组成员统一为「桶标准成员」，`🎯 全球直连` 把 `DIRECT` 提到首位，`🛑 广告拦截` 用 `REJECT`/`DIRECT`/`🚀 节点选择`（5.2.1）。纯函数，无副作用。
- ✅ **A3** `src/utils/rule-generator/parse.js`（约 330 行）
  `INI → GeneratorState`。优先读注释头（无损）；读不到时从 `ruleset=` / `custom_proxy_group=` 尽力反推，返回 `partial: true`。**未被正文提到的内置卡片保留目录原始 sources**、只留在待选栏——这是实现期修掉的一个 bug（附录 C）。小卡片被点亮时其父卡片跟到同桶，保持嵌套关系。另检测"注释头与正文漂移"，返回 `drifted: true`。
...
- ✅ **A6** 测试（89 个用例）
  - `rule-generator-serialize.test.js`（14）— golden file、大卡片多来源合一、小卡片单独拖出、空集合、承接组只生成一组、前置修正指向 DIRECT
  - `rule-generator-roundtrip.test.js`（10）— state → INI → state 幂等、正文反推等价、嵌套关系保持、**未提到的小卡片保留来源**、漂移检测
  - `rule-generator-dedupe.test.js`（18）— 单条/多条撞车、来源粒度去重、归一化边界、入参不被改动
  - `rule-generator-validate.test.js`（30）— 注入字符、`(?i)`、多层括号、重名、跨桶遮蔽、空集合、target 残留被忽略
  - `rule-generator-render-matrix.test.js`（16）— 生成的 INI 过六个渲染器，断言无悬空引用、`MATCH`/`FINAL` 末位、地区组存在与 0 命中被剪除、**一张大卡片只产出 1 个策略组与 N 条规则**、桶组成员不含地区组、`🎯 全球直连` 首位为 `DIRECT`、`🛑 广告拦截` 三成员。写法对齐现有 `builtin-conversion-matrix.test.js`
...
- ✅ **B2** `src/components/modals/RuleGenerator/` 子组件
  ...
  - `CardPalette.vue` — 左栏待选栏（两层嵌套、搜索、散落卡片节）
...
- ✅ **B4** `RuleTemplateManager.vue` 顶部操作栏加「🎨 可视化编辑」入口按钮与回写逻辑
- ✅ **B5** i18n 键补进 `src/i18n/messages.js`（60 键中英双语，对齐现有 `settings.ruleTemplate*` 命名）
- ✅ **B6** 组件测试 `rule-generator-modal.test.js`（17）— 六段可见顺序与输出行序一致、apply 回传而非落盘、漂移警告、大/小卡片拖动语义、自定义规则集落候选区、AD 角标与撞车标红区分

### 阶段 C：后续增强（不进 MVP）

- **C0** **内置目录的分组粒度重做**（新增，优先级最高）。当前策展偏粗：`🎬 流媒体` 7 张小卡片里 5 张默认关闭、`💻 科技服务` 把谷歌与微软/苹果并列但默认落点不同、`🎮 游戏平台` 与 `🌏 广覆盖代理清单` 整个大卡片默认在待选栏。属策展质量而非模型问题，不影响 4.2 的结构。
...
阶段 A + B 已全部满足，`npm run test:run` 116 文件 / 663 用例全绿，`npm run build` 通过。
...
| 注释头随 state 增长可能变大 | 128 KB 上限下余量充足 |
...
| 「🔧 前置修正」的默认落点只覆盖常见情况 | 明确标注为 MVP 权衡；阶段 C3 做完整重叠分析 |
| 目录中的 ACL4SSR URL 可能失效（旧 U2 / U3 未结案） | 已逐条 HEAD 探测（2026-08-30，全部 200）；卡片展开后暴露真实 URL 便于用户自查。仍需在后续版本周期性复查 |
...
| **内置目录分组粒度偏粗** | 已验收确认，列为 C0，属策展质量而非模型问题 |
...
- **ACL4SSR 目录探测结果**：`SteamCN.list` / `GoogleFCM.list` 在 `Clash/Ruleset/` 下（root 为 404）；Disney / GitHub / PayPal / PrimeVideo / GameDownloadCN / Copilot 不存在

---

**文档版本**：2.0（实现同步稿）
**最后更新**：2026-08-31
**状态**：阶段 A + B 已实现并通过本地验收；阶段 C 待排期，C0（目录分组粒度重做）优先
