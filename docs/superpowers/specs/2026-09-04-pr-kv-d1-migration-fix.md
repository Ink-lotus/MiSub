...
**状态**：✅ **已完成**——PR 已提交至上游，等待作者审阅。

| 项 | 值 |
|---|---|
| 上游 PR | **[imzyb/MiSub#474](https://github.com/imzyb/MiSub/pull/474)** —— 「修复 KV→D1 迁移漏搬业务数据导致的静默丢失」，状态 open |
| 分支 | `Ink-lotus:fix/kv-d1-migration` → `imzyb:main`（分支已推送 origin，**不要删**） |
| 基线 | `upstream/main` @ `1008b7c` |
| 提交 | `5038f81`（单个提交，3 文件，+244/−20） |
| 我们自己的仓库 | 同一修复已在 `main` 里（`09e69d8`，随 `38bc1bb` 推送到 origin/main） |

**本文自此转为存档**：正文保留提交 PR 时用的完整材料，末尾「待你决定」三问的最终处置
见文末。后续只需关注上游 #474 的审阅进展，无需再改本文。
...
三问都按上面的倾向落地，PR #474 已按此提交：

| # | 处置 |
|---|---|
| 1 | **提了**。正文「关于测试状态的说明」一节双语说明红灯在基线上即已失败、与本改动无关 |
| 2 | **没顺手修**。保持 PR 小而专，只在正文里提一句「可能需要单独更新该测试」 |
| 3 | **保留在本 PR 里**。新增的搬运键正是那些 `Unknown key format` 告警的源头，拆开反而割裂因果 |

第 2 条留下的尾巴另开了一个 PR 提案：
[`2026-09-05-pr-node-transformer-test-fix.md`](./2026-09-05-pr-node-transformer-test-fix.md)——
单独修那条红灯，1 个文件 4 行。它与本 PR 相互独立，任意顺序提均可。

**同批待提的另一个 PR**：
[`2026-09-05-pr-dns-override-surge-loon-quanx.md`](./2026-09-05-pr-dns-override-surge-loon-quanx.md)
（代码尚未编写）。三者互不依赖。
